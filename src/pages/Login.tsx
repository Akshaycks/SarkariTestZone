...
import { LogIn, Lock, Phone, Send, ShieldCheck } from 'lucide-react';
      ...
      >
        <div className="text-center space-y-2">
          <div className="w-16 h-16 bg-indigo-100 rounded-2xl flex items-center justify-center mx-auto text-indigo-600">
            {isAdminMode ? <ShieldCheck className="w-8 h-8" /> : <LogIn className="w-8 h-8" />}
          </div>
          <h1 className="text-3xl font-bold">{isAdminMode ? 'Admin Login' : 'Welcome Back'}</h1>
          <p className="text-slate-500">{isAdminMode ? 'Access administrative dashboard' : 'Sign in to continue your preparation'}</p>
        </div>
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl">
          <button 
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${!isAdminMode ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Student
          </button>
          <button 
            onClick={() => { setIsAdminMode(true); setOtpSent(false); setError(''); }}
            className={`flex-1 py-2 rounded-lg text-sm font-bold transition-all ${isAdminMode ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}
          >
            Admin
          </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-1">
            <label className="text-xs font-bold text-slate-500 uppercase">{isAdminMode ? 'Email / Phone' : 'Phone Number'}</label>
            <div className="relative flex gap-2">
              <div className="relative flex-1">
                ...
                <input 
                  ...
                  placeholder={isAdminMode ? "admin@prepinsta.com" : "9876543210"}
                  disabled={otpSent && !isAdminMode}
                />
              </div>
              {!otpSent && !isAdminMode && (
                <button 
          ...
          <button type="submit" className="w-full bg-indigo-600 text-white py-4 rounded-xl font-bold hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100">
            {isAdminMode ? 'Login as Admin' : (otpSent ? 'Verify & Login' : 'Send OTP')}
          </button>
        </form>
